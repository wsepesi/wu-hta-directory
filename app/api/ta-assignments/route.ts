...
/**
 * GET /api/ta-assignments
 * Get all TA assignments with optional filters
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Send notification to the assigned TA
    ...
    return NextResponse.json(
      { data: assignment, message: 'TA assignment created successfully' } as ApiResponse<TAAssignment>,
      { status: 201 }
    );
  } catch (error) {
    ...
    if (error instanceof Error && error.message.includes('already exists')) {
      return NextResponse.json(
        { error: 'TA assignment already exists for this user and course offering' } as ApiResponse<never>,
        { status: 409 }
      );
    }
    return NextResponse.json(
